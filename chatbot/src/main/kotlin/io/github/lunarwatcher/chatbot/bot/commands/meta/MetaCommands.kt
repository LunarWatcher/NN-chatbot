...
class RevisionCommand : AbstractCommand("rev", listOf("revision")){
    override fun handleCommand(message: Message): List<ReplyMessage>? {
        return try{
            listOf(ReplyMessage(getRevision() + " (@version ${Configurations.REVISION})", true));
        }catch(e: Exception){
            LogStorage.crash(e)
            listOf(ReplyMessage("Unknown revision", true));
        }
    }
}
...
class GitHubCommand : AbstractCommand("github", listOf("source", "code", "sourceCode", "gh"), desc="Sends the link to GitHub in chat (also available through the about command). " +
    ...
    override fun handleCommand(message: Message): List<ReplyMessage>? = listOf(ReplyMessage("Here you go: ${Configurations.GITHUB}", true));
}
...
class TestCommand : AbstractCommand("test", listOf("items"), desc = "Returns the contents of splitCommand", help = "Play around with the bot"){
    override fun handleCommand(message: Message): List<ReplyMessage>? = listOf(ReplyMessage("Received arguments: " + splitCommand(message.content), false));
}

class LocationCommand : AbstractCommand("location", listOf(), help="Shows the current bot location"){
    override fun handleCommand(message: Message): List<ReplyMessage>? {
        return listOf(ReplyMessage("${Configurations.INSTANCE_LOCATION} (${BotCore.LOCATION})", true));
    }
}
