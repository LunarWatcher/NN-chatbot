...
# TODO improve the performance and memory use of the net, which currently is pretty big
class Bot():
    # Hyperparams
    batchSize = 16
    embedDim = 128
    ...
    def train(self, epochs: int):
        ...
        for epoch in range(epochs):
            ...
            print("Epoch[%d/%d] with average loss:%f took:%.5fs" % (
                epoch + 1, epochs, avgLoss / iterations, time.time() - epochTime))

            tl.files.save_npz(self.net.all_params, name='bot.npz', sess=self.sess)
    ...
    def predict(self, message: str):
        ...
        if not self.training and not self.initialized:
            return "Sorry, I boot asyncronously. The neural net backend boots later than the bot itself. It should be "\
                   "up shortly :) "
    ...
    def startNet(self):
        ...
        if self.training:
            ...
            dataset = "y"  # input("Use the default dataset (y) or a custom one (filename, dataset/ is appended
            # automatically)")
        ...
        if os.path.isfile("dataset/vocabidx2w.npy") and os.path.isfile("dataset/vocabw2idx.npy"):
            ...
            if not self.training:
                print(
                    "The vocab files were not found in the dataset directory. (if changes were made to the "
                    "directories, please change the code as well)")
                print(
                    "Since training is set to false, none of the data required for this operation is present. Please "
                    "train the bot before attempting to use")
                print(
                    "It, or at the very least make sure the vocabs are present. The program will not exit. To avoid "
                    "seeing this warning in the future, please")
            ...
            self.w2idx[
                "name_id"] = self.nameId  # The name ID is the one used in replies to be replaced with the bots name
        ...
        tl.files.load_and_assign_npz(sess=self.sess, name="bot.npz", network=self.net)
...
def parseBoolean(string):
    try:
        ...
        print("Invalid boolean: " + string)
        return False


def getIntInput(prompt, validChoices=None):
    if validChoices is None:
        validChoices = []
    while True:
        try:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--training", help="Am I training? (boolean)")
    parser.add_argument("--mode",
                        help="(When training = false only) Where to boot the bot. 0 is the console, 1 is the flask "
                             "server, which can be used with the Java/Kotlin bot, or a different bot if wanted.")
    ...
    mode = 0
    if not training:
        mode = getIntInput("Mode: ", [0, 1]) if modeArg is None else int(modeArg)
